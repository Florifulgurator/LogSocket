...
synchronized public void onMessage(Session session, String msg) throws Exception {
	...
	if( firstChar=='*' ) {
   	...
   	} // TODO #7e23cb75 Time for an array of lambdas, incl. srvrCommands and frwrdCommands seperate from srvrCommands
...
{
	// Forward command (not necessarily from listener)
	if (lgScktSssn2Nr.k_v.isEmpty()) sendText(sess, "! No LogSockets connected.");
	else sendMsgToAllLogSockets("/PING "+arg);
}
} );
// ---
...
// ---
//srvrCommands.put("%GUI_A", new SrvrCmd() { synchronized public void exec(Session sess, String arg)
//{
//	// Forward command (a /CFT feedback)
//	sendMsgToAllListeners("%GUI_A "+arg, false);
//}
//} );
// ---
srvrCommands.put("/FILTER1_ADD", new SrvrCmd() { public void exec(Session sess, String arg)
{ 
	// Forward command
	sendMsgToAllLogSockets("/FILTER1_ADD "+arg);
}
} );
// ---
...
//---
private static void sendMsgToAllLogSockets(String msg) {
	lgScktSssn2Nr.k_v.forEach( (key,val) -> sendText(key, msg) );
}
