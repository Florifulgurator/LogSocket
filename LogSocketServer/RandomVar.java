...
synchronized public String setValsIntString(String valStr, double unit) {
	...
	Arrays.stream( blankPttrn.split(valStr) ).forEach( x -> {
		try {
			...
			if(errr.isEmpty()) errr = e.toString();
		}
	});
	return errr;
}
String errr = ""; //Local variable errr defined in an enclosing scope must be final or effectively final
...
synchronized private void makeASCIIart(int decPlaces, String unit) {
	...
	dF1.setMaximumFractionDigits(decPlaces+1);dF.setGroupingUsed(false);
...
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
synchronized public long TEST3(long t) { return System.nanoTime()-t; } // Thread sync test/ timing eval

public static class TEST {

public static void main(String[] args) {
	RandomVar r = new RandomVar();
